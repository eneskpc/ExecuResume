...
class CVResolver extends Component {
    ...
    onFileChange = async (e) => {
        ...
        for (let index = 0; index < files.length; index++) {
            if (files[index].name.substring(files[index].name.length - 3).toLowerCase() === 'pdf' ||
                files[index].name.substring(files[index].name.length - 3).toLowerCase() === 'doc' ||
                files[index].name.substring(files[index].name.length - 4).toLowerCase() === 'docx') {
    ...
    onSubmit = async (dispatch, e) => {
        ...
        for (let index = 0; index < cvFiles.length; index++) {
            let receivedResponse = await axios.post(`${ServerAddress}/api/resume`, {
                                    ...
                                    <form onSubmit={this.onSubmit.bind(this, dispatch)} className="row">
                                        ...
                                        <div className="col-12 d-md-flex justify-md-content-between align-items-md-center text-md-left text-center">
                                            <div className="custom-file">
                                                <input multiple type="file" className="custom-file-input" onChange={this.onFileChange} />
